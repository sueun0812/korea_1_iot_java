...
class EventManagement {
	...
	// 1. 참가자 명단에 참가자 추가
	void addParticipantList(String name) {
	...
	void leaveParticipant(String name) {
		...
		if (waitingQueue.size() > 0) {
			...
			addParticipantList(nextParticipant);
		}
	}
...
public class C_ListPractice {
	public static void main(String[] args) {
		...
		// 참가자 명단에 참가자 추가
		event.addParticipantList("이승아"); 
		event.addParticipantList("이도경"); 
		event.addParticipantList("이주헌"); 
		event.addParticipantList("이지희"); 
		event.addParticipantList("이지훈");
